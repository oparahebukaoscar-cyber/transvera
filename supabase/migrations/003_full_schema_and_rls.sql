...
CREATE POLICY audit_logs_admins_select ON public.audit_logs FOR SELECT
  USING (
    (SELECT is_super_admin FROM public.profiles WHERE id = auth.uid()) = true
    OR EXISTS (
      SELECT 1 FROM public.organization_members om
      WHERE om.profile_id = auth.uid() AND om.role IN ('owner','admin') AND om.status = 'active'
    )
  );

-- 5) Final notes
-- The telemetry ingestion path should be performed by backend workers or a
-- SECURITY DEFINER function (not by arbitrary clients) so that telemetry inserts
-- are performed with elevated privileges (service role) rather than relying on
-- broad INSERT policies.
